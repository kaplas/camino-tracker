...
const earliestDate = moment("2017-04-07 12:00");
...
const getPlacesAsync = entries => Bluebird.resolve(entries)
  ...
  .then(res => res.json())
  .then(res => res && res.Response && res.Response.Item);
...
Bluebird.resolve(true)
  ...
  // Filter problematic and unwanted entries away
  ...
  .catch(err => {
    console.error("Error: JSON fetching from Google Sheets failed");
